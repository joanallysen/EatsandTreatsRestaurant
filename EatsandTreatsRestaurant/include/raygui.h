...
void GuiLoadStyle(const char *fileName)
{
    ...
    FILE* rgsFile = nullptr;
    errno_t err = fopen_s(&rgsFile, fileName, "rb");
...
char **GuiLoadIcons(const char *fileName, bool loadIconsName)
{
    ...
    FILE* rgiFile = nullptr;
    errno_t err = fopen_s(&rgiFile, fileName, "rb");

    if (err != 0) {
        // Handle the error (file couldn't be opened)
        printf("Error opening file: %d\n", err);
    }
    else {
        // Successfully opened the file, you can now read from it
        printf("File opened successfully!\n");
    }
